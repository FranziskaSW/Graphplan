...
def independent_pair(a1, a2):
    """
    Returns true if the actions are neither
    - have inconsistent effects nor
    ...
    # interfere:
    interfere_12 = any([a2.is_pre_cond(p) for p in a1.get_delete()])  # a1 deletes something that is precond for a2
    interfere_21 = any([a1.is_pre_cond(p) for p in a2.get_delete()])

    if any([inconsist_eff_12, inconsist_eff_21, interfere_12, interfere_21]):
        return False
    return True
